...
network_data = fund_n[which(fund_n$date %in% c(date)),]
next_network_data = fund_n[which(fund_n$date %in% c(next_date)),]
...
nw_activist = next_network_data[which(next_network_data$activist == activist_cik),]
...
if( ending.quarter ==  next_date){ nw_activist$time_end = 1 }else{  nw_activist$time_end = 0 }
}

nw_activist_connect = nw_activist[ which(nw_activist$activist %in% connect_list),  ]
if( nrow(nw_activist_connect) >0 ){
  nw_activist_connect$in_same_company=1
}

nw_activist_not_connect =  nw_activist[ which(nw_activist$activist %!in% connect_list),  ]
...
nw_not_active_activist = next_network_data[which(next_network_data$activist %in% not_active_activists),]
nw_not_active_activist$campaign_id = c
nw_not_active_activist$treated = 0
nw_not_active_activist$win = campaign_outcome
if( ending.quarter ==  next_date){ nw_not_active_activist$time_end = 1 }else{  nw_not_active_activist$time_end = 0 }
if( nrow(nw_not_active_activist) >0 ){
  nw_not_active_activist$in_same_company=0
}

out = rbind(nw_activist, nw_not_active_activist)
...
#------------------------Do the same controlling activist herself when not active
